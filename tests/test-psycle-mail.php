...
class MailOptionsTest extends PHPUnit_Framework_TestCase {
	...
	public function test_callback_settings_field() {
		...
		$fields = array(
			'from' =>
				...
				array(
					...
					'default' => 'Webmaster at example.com',
				),
				'return_path' =>
				array(
					'type' => 'email',
		...
		foreach ( $fields as $name => $option_field ) {
			...
			switch ( $name ) {
				case 'from':
					$expected = '<phpunitroot><input type="email" name="_psycle_mail_from" value="webmaster@example.com" size="60"><p class="description">The From email address for the message.</p></phpunitroot>';
					break;
				case 'from_name':
					$expected = '<phpunitroot><input type="text" name="_psycle_mail_from_name" value="Webmaster at example.com" size="60"><p class="description">The From name of the message.</p></phpunitroot>';
					break;
				case 'return_path':
					$expected = '<phpunitroot><input type="email" name="_psycle_mail_return_path" value="webmaster@example.com" size="60"><p class="description">The Return-Path of the message.&lt;br&gt;If empty, it will be set to either From or Sender.</p></phpunitroot>';
					break;
				case 'sender':
					$expected = '<phpunitroot><input type="email" name="_psycle_mail_sender" value="webmaster@example.com" size="60"><p class="description">The Sender email (Return-Path) of the message. &lt;br&gt;If not empty, will be sent via -f to sendmail or as &#039;MAIL FROM&#039; in smtp mode. This will become the &#039;Reply-To&#039; address if one isn&#039;t already set.</p></phpunitroot>';
					break;
				case 'reply_to_name':
	...
	public function test_action_php_mailerinit() {
		...
		$this->assertContains( 'From: "Webmaster at example.com" <webmaster@example.com>', $headers );
		$this->assertContains( 'Reply-To: "Webmaster at example.com" <validsender@example.com>', $headers );
	}
